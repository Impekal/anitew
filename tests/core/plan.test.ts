...
const plan = (mode: (typeof TRAINING_MODES)[number], seed = 'test', due: string[] = []) =>
  planSession({
    ...
    language: 'de',
    seed,
    pools: { words: pool, faces: [], numbers: [], missions: [], palace: [], reverse: [], twins: [], gaze: [], facts: [], memory: [] },
    due: { words: due },
    modules: ['words'],
  })
  ...
  it('sagt deutlich Bescheid, wenn der Vorrat nicht reicht', () => {
    expect(() =>
      planSession({
        ...
        language: 'de',
        seed: 'x',
        pools: { words: ['eins', 'zwei'], faces: [], numbers: [], missions: [], palace: [], reverse: [], twins: [], gaze: [], facts: [], memory: [] },
        modules: ['words'],
      }),
    ).toThrow(RangeError)
  })
