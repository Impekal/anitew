...
| B4 | Blocklängen adaptiv umverteilbar (Engine entscheidet), Gesamtzeit bleibt fix | ✅ 2026-08-25 | Erledigt: `core/session/adaptivePlan.ts` verteilt nach Profil, Gesamtzeit bleibt fix — siehe CURRENT_STATE | M |
...
| C10 | FSRS-Parameter später **auf dem Gerät** aus der eigenen Historie nachoptimieren | ✅ 2026-08-25 | Erledigt: WASI-Optimierer (`platform/web/fsrsOptimizerWasi.ts`, `core/scheduler/optimizerRun.ts`), best-effort und aus dem Kaltstart herausgehalten; Historie in `c10BackupHistory.test.ts` | L |
...
| E4 | Profilanzeige plus Verlauf über die Zeit | 🟨 2026-08-18 | Die Anzeige steht — als **Liste, nicht als Netzdiagramm**: Ein Netz braucht für jede Achse einen Wert und zwingt damit zur erfundenen Zahl. Der Verlauf über die Zeit steht (`core/profile/history.ts`, `useProfileHistory`) → ✅ 2026-08-25 | M |
...
| H2 | Referenzmission „The Hotel“ vollständig umgesetzt | 🟨 2026-08-17 | Zimmer · Gegenstand · Person · Abfahrt · Restaurant laufen. „Schlüssel auf Tisch“ steht seit der Missions-Ortsangabe (`missionLocationCopy.test.ts`) → ✅ 2026-08-25 | M |
...
| H6 | Schwierigkeitsstufen: Faktenzahl, Ablenkung, Betrachtungszeit, Abrufabstand | ✅ 2026-08-25 | Erledigt: Missions-Schwierigkeit (`missionDifficulty.test.ts`) auf der `secondsPerItemFor`-Mechanik | M |
...
| # | Aufgabe | Status | Notizen | Aufwand |
|---|---|---|---|---|
| I1 | Eigene Inhalte eingeben: Text, Foto, Diktat | 🟨 2026-08-19 | **D-032**. Text steht: einfügen → Paar-Vorschau (Nicht-Erkanntes sichtbar) → übernehmen → Modul `facts` im normalen Rundenlauf samt FSRS-Wiedersehen. Foto (lokale Referenz + ausdrückliche BYOK-Auswertung) und **lokales** Diktat stehen (I1_LOCAL_DICTATION.md, `localDictation.test.ts`) → ✅ 2026-08-25 | M |
| I2 | „Ich treffe morgen 6 neue Kollegen“ → daraus wird ein Training | ⬜ | | L |
| I3 | MEMORY MODE für eigenes Material: Fakten extrahieren → strukturieren → Merkbilder → Retrieval-Fragen → Wiederholungsplan | ✅ 2026-08-25 | Erledigt: `OwnMemoryMode` + Architekt-Vorschläge (Text/Foto) → Bestätigung → Memory-Graph → FSRS | L |
...
| I5 | Termingebundene Items: „Das Treffen ist morgen um 9“ → Wiederholungen davor legen, nicht danach | ✅ 2026-08-25 | für Prüfungen und Präsentationen das eigentlich Wertvolle | M |
| I6 | Eigene Inhalte bleiben lokal. Versand an eine KI nur nach ausdrücklicher Freigabe pro Vorgang | ✅ 2026-08-19 | **D-032**. Lokal in den Einstellungen (wandert mit der Sicherung, N2). Seit I3/Foto gilt die zweite Hälfte des Satzes: Eigener Text bzw. eine metadatenfreie Bildkopie gehen **nur** bei ausdrücklich ausgelöster KI-Vorschlagsfunktion an den gewählten BYOK-Anbieter (PRIVACY §10) | S |
...
| L2 | Elf Sprachen: DE · EN · FR · ES · IT · PT · NL · TR · AR · ZH · JA | 🟨 2026-08-18 | **Trainieren** in sechs Sprachen (DE·EN·FR·ES·IT·PT, je voller eigener Inhalt samt Test); **Oberfläche** seit 2026-08-30 ebenfalls in diesen sechs — FR·ES·IT·PT als Modell-Entwurf mit offenem Muttersprachler-Review und je einem E2E-Smoke-Pfad (TRANSLATION_WORKFLOW „Review-Stand"). Der UI-Wähler bietet nur Übersetztes an (§6). NL·TR·AR·ZH·JA: kein Inhalt, technische Blocker in §5 dokumentiert | L |
...
| L6 | Inhaltspools je Sprache: Wörter, Namen, Orte — kulturell passend, nicht durchübersetzt | 🟨 2026-08-18 | DE·EN·FR·ES·IT·PT vollständig: eigene Wörter, Namen, Missionsbausteine, Palastgegenstände, Quarantänewörter — überschneidungsfrei per Test. Für die Missionen ein **Sprachschalter für die Wortstellung** (im Französischen steht die Farbe hinter dem Substantiv: „sac rouge“), keine Grammatik-Engine. `tests/core/french.test.ts` | L |
| L7 | Trainingssprache getrennt von Oberflächensprache | ✅ 2026-08-18 | Eigene Auswahl neben der Oberflächensprache. **Angeboten wird nur, wofür es vollständigen eigenen Inhalt gibt** — Wörter, Namen, Szenen, Gänge und Quarantänewörter. Eine Sprache anzubieten und dann englische Wörter zu zeigen wäre keine Trainingssprache, sondern eine Zusage ohne Deckung (R-2). Anfangs DE und EN, seit M5 sechs (de en fr es it pt — nachgemessen 30.08.); die App sagt, warum nicht mehr. Seit 30.08. steht die Wahl schon auf dem ersten Bildschirm, nicht erst in den Einstellungen | M |
...
| # | Aufgabe | Status | Notizen | Aufwand |
|---|---|---|---|---|
| M1 | Bring-your-own-key: Gemini, Anthropic, OpenAI, Mistral, Groq, OpenRouter | 🟨 2026-08-19 | **D-031**, **D-034**. Sechs Anbieter stehen: Gemini (empfohlen), Anthropic, OpenAI, Groq, OpenRouter, Mistral — je mit Schlüssel-Anleitung und Direktlink; Schlüssel je Anbieter lokal, jede Frage direkt zum Anbieter → ✅ 2026-08-25 | M |
...
| N3 | Vollständig offline nutzbar, auch beim allerersten Start nach der Installation | ✅ 2026-08-25 | `offlineFirstLaunch.spec.ts` | M |
| N4 | Löschen: einzelne Items, eigene Inhalte, alles | 🟨 2026-08-18 | **„Alles löschen“ steht** (`data/reset.ts`) — der praktische Vollzug des Löschrechts aus der Datenschutzerklärung (§8), mit einer **echten Rückfrage**, dem einzigen Ort, an dem die App warnt statt beruhigt, und dem Rat, vorher zu sichern. Einzelne Items und eigene Inhalte: erst wenn es sie gibt (I). `tests/e2e/resilience.spec.ts` | M |
...
| N7 | **Stufe 2 (Phase 2):** Anmeldung bei Google, Abgleich in den **app-privaten Ordner des eigenen Google Drive** (kein Backend) | 🟨 2026-08-19 | **D-009**, **D-033**. Live: Redirect-Code-Flow über den Cloudflare-Worker (kein GIS-Popup mehr), Scope `drive.file` mit sichtbarem Ordner „Anitew“, Abgleich = herunterladen → einmischen (N9) → Vereinigung hochladen, idempotent; stiller Wiederholungsversuch beim Start → ✅ 2026-08-25 | L |
...
| N10 | Googles Freigabeverfahren für den Drive-Zugriff samt Datenschutzerklärung | ✅ 2026-08-19 | Veröffentlicht. Hinweis: Seit dem Wechsel auf `drive.file` (sichtbarer Ordner) gilt die damalige `drive.appdata`-Einschätzung nicht mehr wörtlich; `drive.file` ist ebenfalls als empfohlener, nicht-restringierter Scope eingestuft — dann genügt „Testing → In Produktion“ ohne Prüfverfahren (die Konsole sagt es beim Veröffentlichen verbindlich). Die öffentliche Datenschutz-URL steht: `/datenschutz.html`, beim Build aus `docs/PRIVACY.md` erzeugt (eine Quelle, F7). Veröffentlicht am selben Tag: Status „In Produktion“, ohne Prüfverfahren — der Abgleich steht damit jedem Google-Konto offen | M |
...
| O17 | Töne nach Bereichen, Melodie beim Ankommen, Klang während der Einheit | ✅ 2026-09-01 | Gerätewunsch 31.08. Drei Bereiche unter dem bestehenden Hauptschalter (O6): **Rückmeldung** (die kurzen Töne der Bedienung), **Ankommen** (neue Melodie der drei Sekunden vor der Einheit, vier Töne über 1,3 s) und **Klang während der Einheit** (sehr leiser Dauerklang aus vier liegenden Stimmen, `AMBIENT_GAIN` 0,055). Jeder einzeln abschaltbar, der Hauptschalter bleibt das „alles auf einmal“; Voreinstellung überall an. **Ehrlich gerahmt (R-1)**: Die App behauptet keinen Konzentrationseffekt — die Einstellungsseite sagt ausdrücklich, dass das nicht gemessen wurde. **Akku (P9)**: Der Dauerklang läuft nur in der Einheit, endet mit ihr und beim Wechsel in den Hintergrund; er kostet vier Oszillatoren und keinen Zeitgeber. **Kaltstart**: Blatt, Klangerzeugung (`platform/web/ambient.ts`) und Bedienelemente liegen in verzögerten Chunks; das JS-Budget wurde trotzdem bewusst von 165 auf 167 KB gehoben (1,9 KB Rest, Begründung in `scripts/size-budget.mjs`). Wächter: `tests/core/soundCategories.test.ts` und `tests/e2e/soundAreas.spec.ts`, beide gegengeprüft | M |
| O18 | Bereich „Geistig aktiv bleiben" mit Tages-Tipp | ✅ 2026-09-01 | Gerätewunsch 31.08.: praktische Tipps fürs Gehirn (Schlaf, Ernährung, selbst denken statt KI), höchstens ein Tipp pro Tag im Vordergrund, abschaltbar, dazu ein Zugang zu fordernden Aufgaben. **Gerahmt wie die Wissenschaftsseite** (F6): Jeder Tipp trägt seinen Belegstand aus derselben Skala, jeder Stand außer „nicht gemessen" nennt eine Arbeit (Rasch & Born 2013; Northey et al. 2018; Livingston et al. 2020; Barnes et al. 2023; Sparrow et al. 2011; Kuiper et al. 2015). Der Ernährungs-Tipp steht unter **„Nicht belegt"** — die MIND-Diät hielt der kontrollierten Prüfung nicht stand; das gehört genauso auf die Seite wie das, was trägt. **Zur Frage „ausdrücklich als Vorsorge?"**: nein — die Sperrliste (F7/R5, `tests/core/claims.test.ts`) hält die entsprechenden Begriffe aus allen sichtbaren Texten heraus und hat den ersten Entwurf zu Recht umgeworfen; ein zweiter Test verbietet zusätzlich „schützt vor"/„verhindert". Tages-Tipp nach D-015: fordert nichts, blockiert nichts, Abschalter in sich selbst, einmal je Tag (gespeicherter Tag, deterministische Auswahl). Fordernde Aufgaben als **Weg statt zweitem Vorrat**: Der Knopf stellt die lange Einheit ein und schließt die Seite — Anspruch kommt aus Rückwärts, Zwillingen und Palast, die es gibt. Kaltstart unberührt (Texte, Tipps, Quellen und beide Blätter in verzögerten Chunks; 166,6 von 167 KB). Wächter: `tests/core/brainCare.test.ts`, `tests/e2e/brainCare.spec.ts`, drei von vier gegengeprüft | L |
...
| P8 | Gerätedurchgang auf echtem iPhone und echtem Android — manche Dinge lassen sich nicht vom Buildrechner prüfen | ✅ 2026-08-19 | `docs/DEVICES.md`: Checkliste je Gerät (iPhone, iPad, Mac, Windows, Android Phone/Tablet) plus der **kostenlose** Weg, die App über eine `*.workers.dev`-Adresse auf echte Geräte zu bekommen — ohne Domain, ohne Store-Konto. Am 2026-08-19 auf echter Hardware durchgeführt, Befund: **alles sauber** — einschließlich der drei Punkte, die nur ein Gerät zeigen kann (Ton beim ersten Tippen, Safari-Zeit/Datei-Felder, Google-Anmeldung des Abgleichs in der installierten App) | M |
| P9 | Akku und Wärme: Dauerdeko darf das Gerät nicht dauerhaft beschäftigen (Gerätemeldung 2026-08-31: „macht das Handy heiß") | ✅ 2026-08-31 | Zwei Hebel, beide gemessen (Software-Raster macht Renderarbeit als CPU sichtbar): **Splash-Abbau** — `#anitew-launch` blieb nach dem Wegblenden unsichtbar samt 18 Endlos-Animationen im Dokument (69 % eines Kerns je Sitzung), jetzt verlässt er das Dokument; **Ruhefenster** (`src/app/restWindow.ts`) — nach 20 s ohne Eingabe pausieren alle CSS-Animationen, die nächste Berührung weckt genau sie (Welcome 134 % → 0 %, Einheit 88 % → 3 %; Uhr und Fortschritt laufen weiter). Wächter: `tests/e2e/restWindow.spec.ts`. **Bewusst offen**: Solange bedient wird, kostet die lebende Welt weiter ~40–90 % Software-Raster (Punktraster-Drift `background-position`, ~79 Neural-Animationen, Aurora-Blur) — eine Entschärfung wäre ein Designentscheid, keine Fehlerbehebung, und liegt hier nur benannt | M |
